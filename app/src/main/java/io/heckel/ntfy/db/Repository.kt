...
class Repository(private val sharedPrefs: SharedPreferences, database: Database) {
    ...
    fun getMarkdownEnabled(): Boolean {
        return sharedPrefs.getBoolean(SHARED_PREFS_MARKDOWN_ENABLED, true) // Render all messages as Markdown by default
    }

    fun setMarkdownEnabled(enabled: Boolean) {
        sharedPrefs.edit {
            putBoolean(SHARED_PREFS_MARKDOWN_ENABLED, enabled)
        }
    }
    ...
    companion object {
        ...
        const val SHARED_PREFS_MARKDOWN_ENABLED = "MarkdownEnabled"
